[`< Back`](./)

---

# AppraisersContactDetails

Namespace: BurnSoft.Applications.MGC.Types

Container for the Appraisers_Contact_Details table

```csharp
public class AppraisersContactDetails
```

Inheritance [Object](https://docs.microsoft.com/en-us/dotnet/api/system.object) → [AppraisersContactDetails](./burnsoft.applications.mgc.types.appraiserscontactdetails.md)<br>
Attributes [SerializableAttribute](https://docs.microsoft.com/en-us/dotnet/api/system.serializableattribute)

## Properties

### **Id**

Gets or sets the identifier.

```csharp
public int Id { get; set; }
```

#### Property Value

[Int32](https://docs.microsoft.com/en-us/dotnet/api/system.int32)<br>
The identifier.

### **Name**

Gets or sets the name this column is called aName in the table

```csharp
public string Name { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The name.

### **Address1**

Gets or sets the address1.

```csharp
public string Address1 { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The address1.

### **Address2**

Gets or sets the address2.

```csharp
public string Address2 { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The address2.

### **City**

Gets or sets the city.

```csharp
public string City { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The city.

### **State**

Gets or sets the state.

```csharp
public string State { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The state.

### **Country**

Gets or sets the country.

```csharp
public string Country { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The country.

### **Phone**

Gets or sets the phone.

```csharp
public string Phone { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The phone.

### **Fax**

Gets or sets the fax.

```csharp
public string Fax { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The fax.

### **Email**

Gets or sets the email.

```csharp
public string Email { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The email.

### **Lic**

Gets or sets the lic.

```csharp
public string Lic { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The lic.

### **WebSite**

Gets or sets the web site.

```csharp
public string WebSite { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The web site.

### **ZipCode**

Gets or sets the zip code.

```csharp
public string ZipCode { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The zip code.

### **StillInBusiness**

Gets or sets a value indicating whether [still in business].

```csharp
public bool StillInBusiness { get; set; }
```

#### Property Value

[Boolean](https://docs.microsoft.com/en-us/dotnet/api/system.boolean)<br>
`true` if [still in business]; otherwise, `false`.

### **LastSync**

Gets or sets the last synchronize. This is called sync_lastupdate in the table.

```csharp
public string LastSync { get; set; }
```

#### Property Value

[String](https://docs.microsoft.com/en-us/dotnet/api/system.string)<br>
The last synchronize.

## Constructors

### **AppraisersContactDetails()**

```csharp
public AppraisersContactDetails()
```

---

[`< Back`](./)
